...
class VivadoClient:
    ...
    @_command()
    def source_tcl(self, files):
        cmd = "source {{{:s}}}\r".format(" ".join(files))
        self.socket.sendall(cmd.encode())

    @_command()
    def synth_design(self, top, part):
        self.socket.sendall(
            "synth_design -top {:s} -part {:s}\r".format(top, part).encode()
        )
    ...
    def set_property(self, prop_dict, objects):
        dict_string = " ".join(
            ["{} {}".format(key, val) for key, val in prop_dict.items()]
        )
    ...
    def create_hw_bitstream(self, filename):
        cmd = "create_hw_bitstream -hw_device [current_hw_device] {:s}\r".format(
            filename
        )
        self.socket.sendall(cmd.encode())
    ...
    def _report_drc(self):
        self.socket.sendall(
            b"report_drc -no_waivers -upgrade_cw "
    ...
    def _handle_report(self, response, name):
        header = r"^\d+\..+\r\n-+\r\n"
        pattern = r"{}[\s\S]+?(?=(?:{})|^.+completed successfully)".format(
            header, header
        )
    ...
    def _report_methodology(self):
        self.socket.sendall(
            b"report_methodology -no_waivers -checks [get_methodology_checks]\r"
        )
    ...
    def _report_timing(self):
        self.socket.sendall(
            b"report_timing -delay_type min_max -max_paths 1 -slack_less_than 0\r"
        )
...
def main():
    ...
    parser_parent.add_argument(
        "--host", default="localhost", help="A hostname to which to connect."
    )
    parser_parent.add_argument(
        "--port", type=int, default=9191, help="A port number for connection."
    )
    ...
    parser_synth = subparsers.add_parser(
        "synth", parents=[parser_parent, parser_output], help="Synthesize design."
    )
    parser_synth.add_argument(
        "-v", "--verilog", nargs="+", required=True, help="Verilog file."
    )
    parser_synth.add_argument(
        "-t", "--top", required=True, help="Top level module name."
    )
    ...
    # Bitstream Command.
    parser_bitstream = subparsers.add_parser(
        "bitstream",
        parents=[parser_parent, parser_input, parser_output],
        help="Write bitstream.",
    )
    parser_bitstream.add_argument(
        "--check", action="store_true", help="Perform check on design."
    )
    ...
    parser_cfg_mem = subparsers.add_parser(
        "cfg_mem",
        parents=[parser_parent, parser_input, parser_output],
        help="Write config memory file.",
    )
    parser_cfg_mem.add_argument(
        "--size", type=int, required=True, help="Memory size [MB]."
    )
    parser_cfg_mem.add_argument(
        "--interface", choices=valid_interfaces, required=True, help="Memory size [MB]."
    )
    parser_cfg_mem.set_defaults(func=cfg_mem)

    # Load Command.
    parser_load = subparsers.add_parser(
        "load", parents=[parser_parent, parser_input], help="Load bitstream."
    )
    ...
    parser_flash.add_argument(
        "--memory", required=True, help="Vivado memory part number."
    )
    parser_flash.set_defaults(func=flash)

    # Check Command.
    parser_check = subparsers.add_parser(
        "check", parents=[parser_parent, parser_input], help="Run design checks."
    )
